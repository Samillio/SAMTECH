  ...
  return (
    <nav className="bg-green-800">
      ...
      <div className="flex justify-between items-center px-4 py-3">
        ...
        <Link to="/" className="text-yellowish font-bold">Sirmillion Solutions</Link>
        {/* For small screens, show the hamburger menu icon */}
        <div className="md:hidden" onClick={toggleMenu}>
          <svg
            className="h-6 w-6 text-yellowish cursor-pointer"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      <ul className={`md:flex md:items-center md:space-x-4 ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <li>
          <Link to="/register" className="block px-4 py-2 text-white hover:bg-gray-700" onClick={toggleMenu}>Subscribe</Link>
        </li>
      </ul>
    </nav>
  );
